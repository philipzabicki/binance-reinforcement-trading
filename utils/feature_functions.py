...
def add_candle_patterns_indicators(df):
    ...
    for pattern_name in candle_names:
        ...
        df[pattern_name] = pattern_function(
...
def change_kde_pdf(df, column="Close", start_index=10, cpus=-1):
    ...
    probs_list = Parallel(n_jobs=cpus, backend="threading")(
        delayed(_compute_prob_t)(t, diffs[t], increases[:t], decreases[:t])
        for t in range(start_index, diffs.shape[0])
    )
    df[f"{column}_change_KDE_gauss"] = np.concatenate(
        (np.zeros(start_index), probs_list)
    )
    return df
...
def candle_sizes_kde_pdf(df, start_index=10, cpus=-1):
    ...
    for f in [
        "candle_size",
        "candle_body_size",
        "candle_upshadow_size",
        "candle_downshadow_size",
        "candle_body_ratio",
        "candle_upshadow_ratio",
        "candle_downshadow_ratio",
    ]:
        candle_part = np.ascontiguousarray(df[f].values)
        probs_list = Parallel(n_jobs=cpus, backend="threading")(
            delayed(_compute_prob_candle)(t, candle_part[: t + 1])
            for t in range(start_index, df.shape[0])
        )
...
def candle_sizes_kde_pdf_kernels(df, kernels=["gau"], start=10, cpus=-1):
    def _compute_prob_candle_kernel(t, vals_up_to_t, kernel="gau"):
        ...
        cdf_interp = interp1d(
            grid_points, cdf_values, bounds_error=False, fill_value=(0, 1)
        )
    ...
    for kernel in kernels:
        for f in [
            ...
            probs_list = Parallel(n_jobs=cpus, backend="threading")(
                delayed(_compute_prob_candle_kernel)(
                    t, candle_part[: t + 1], kernel=kernel
                )
                for t in range(start, df.shape[0])
            )
