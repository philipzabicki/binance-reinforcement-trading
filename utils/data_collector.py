...
def get_precalculated_dataset_by_filename(dataset_filename, train_test_split_date):
    ...
    if train_test_split_date is not None:
        return (
            df[(df["Opened"] <= train_test_split_date)],
            df[(df["Opened"] > train_test_split_date)],
        )
    else:
        return df
...
def _fix_and_fill_df(df, itv):
    ...
    df.iloc[:, -1] = df.iloc[:, -1].replace(0.0, 0.00000001)
    return df
...
def by_BinanceVision(
    ...
    data_frames += _collect_to_date(
        url, output_folder, start_date=_start_date, delta_itv="days"
    )
    ...
    if path.isfile(file):
        ...
        if time() - last_timestamp > delay:
            ...
            DataClient(futures=futures).kline_data(
                [ticker.upper()],
                interval,
                storage=["csv", DATA_DIR + directory],
