...
ACTIONS_FULLPATH = os.path.join(
    REPORT_DIR, "optimal_actions", "final_combined_actions_vwap.csv"
)
...
def pool_initializer(ohlcvvwap_np):
    keltner_channel_initializer(ohlcvvwap_np)


def run_part(df_h: pd.DataFrame,
             half_idx: int,
             ohlcvvwap_np: np.ndarray,
...
def main(n_splits=2, max_iterations=MAX_ITERATIONS):
    ...
    print(df.describe())
    ohlcvvwap_np = df.to_numpy()[:, 1:7].astype(float)
    print(f'All segments {int((df["Action"] != 0).sum()) // 2}')
    trange_arr = TRANGE(*df.to_numpy()[:, 2:5].T.astype(float))

    if n_splits == 0:
        print("n_splits=0 -> using the full dataframe (no zeroing).")
        run_part(df.copy(), half_idx=0, ohlcv_np=ohlcvvwap_np, trange_arr=trange_arr, max_iterations=max_iterations)
        return
    ...
    for b_idx in range(n_splits):
        ...
        run_part(df_b, half_idx=b_idx, ohlcvvwap_np=ohlcvvwap_np, trange_arr=trange_arr, max_iterations=max_iterations)
