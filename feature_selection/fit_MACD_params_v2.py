...
TERMINATION = DefaultMultiObjectiveTermination(
    ftol=0.001, period=5, n_max_gen=10, n_max_evals=1_000_000
)
...
def main(max_iterations=10):
    ...
    with Pool(CPU_CORES_COUNT, initializer=pool_initializer, initargs=(np_df,)) as pool:
        ...
        while unmatched_segments.shape[0]:
            ...
            problem = PROBLEM(df, unmatched_segments, elementwise_runner=runner)
        ...
        output_file = os.path.join(
            RESULTS_DIR, f"macd_pop{POP_SIZE}_iters{MAX_ITERATIONS}.csv"
        )
