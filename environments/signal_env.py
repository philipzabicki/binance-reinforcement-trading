...
class SignalExecuteFuturesEnv(FuturesBacktest):
    def __init__(self, *args, **kwargs):
        ...
        self.stop_loss = kwargs["stop_loss"] if "stop_loss" in kwargs else None
        self.take_profit = kwargs["take_profit"] if "take_profit" in kwargs else None
        self.long_enter_threshold = (
            kwargs["long_enter_at"] if "long_enter_at" in kwargs else 1.0
        )
        self.long_close_threshold = (
            kwargs["long_close_at"] if "long_close_at" in kwargs else 1.0
        )
        self.short_enter_threshold = (
            kwargs["short_enter_at"] if "short_enter_at" in kwargs else 1.0
        )
        self.short_close_threshold = (
            kwargs["short_close_at"] if "short_close_at" in kwargs else 1.0
        )
        self.signals = choice([-1, 0, 1], size=self.total_steps)
    ...
    def _finish_episode(self):
        ...
        if self.verbose:
            print(
                f" position_ratio={self.position_ratio:.2f}, leverage={self.leverage}"
            )
            if (self.take_profit is not None) and (self.stop_loss is not None):
                print(
                    f" stop_loss={self.stop_loss * 100:.3f}%, take_profit={self.take_profit * 100:.3f}%"
            ...
            print(
                f" long_enter_at={self.long_enter_threshold:.3f}, long_close_at={self.long_close_threshold:.3f}"
            )
            print(
                f" short_enter_at={self.short_enter_threshold:.3f}, short_close_at={self.short_close_threshold:.3f}"
            )
