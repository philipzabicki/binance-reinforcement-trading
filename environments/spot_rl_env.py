...
class SpotTakerRL(SpotBacktest):
    ...
    def reset(self, **kwargs):
        ...
        trade_data = [self.qty, self.balance/self.init_balance, self.position_size/self.balance,
    ...
    def _next_observation(self):
        ...
        dist_to_sl = (self.df[self.current_step, 3] - self.stop_loss_price)/self.df[self.current_step, 3] if self.stop_loss is not None else 0
        dist_to_tp = (self.df[self.current_step, 3] - self.take_profit_price)/self.df[self.current_step, 3] if self.take_profit is not None else 0
        trade_data = [self.qty, self.balance/self.init_balance, self.position_size/self.balance,
                      self.in_position, self.in_position_counter, self.pnl,
...
class SpotMakerRL(SpotBacktest):
    def __init__(self, df, *args, **kwargs):
        ...
        raise NotImplemented('Market-maker version of this environment is not implemented yet.')