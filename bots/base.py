...
class SpotTaker:
    def __init__(
        ...
        self.buy_slipp_file = (
            f"{SLIPPAGE_DIR}{market}/{self.symbol}{itv}/market_buy.csv"
        )
        self.sell_slipp_file = (
            f"{SLIPPAGE_DIR}{market}/{self.symbol}{itv}/market_sell.csv"
        )
        self.stoploss_slipp_file = (
            f"{SLIPPAGE_DIR}{market}/{self.symbol}{itv}/limit_stop_loss.csv"
        )
        ...
        prev_candles = self.client.get_historical_klines(
            self.symbol, itv, str(prev_size * multi) + " minutes ago UTC"
        )
        prev_data = array(
            [array(list(map(float, candle[1:6]))) for candle in prev_candles[:-1]]
        )
        prev_data[where(prev_data[:, -2] == 0.0), -2] = 0.00000001
        self.OHLCV_data = deque(prev_data, maxlen=len(prev_candles[:-1]))
        self.close = self.OHLCV_data[-1][3]
        self.init_balance = float(
            self.client.get_asset_balance(asset=self.quote)["free"]
        )
    ...
    def on_message(self, ws, message):
        ...
        if data_k["x"]:
            ...
            fixed_volume = (
                0.00000001 if float(data_k["v"]) <= 0.0 else float(data_k["v"])
            )
            ...
            print(
                f"INFO close:{self.close:.2f} bal:${self.balance:.2f} q:{self.q}",
                end=" ",
            )
            print(f"cum_pnl:${self.cum_pnl:.2f}")
        if float(data_k["l"]) <= self.stoploss_price:
            _order = self.client.get_order(
    ...
    def _analyze(self):
        ...
        if (self.signal >= self.enter_at) and (self.balance > 5.01):
            q = str((self.balance / self.close) - 0.00001)[:7]
    ...
    def _partially_filled_problem(self):
        ...
        if self._market_sell(self.q):
            self._report_slipp(
                self.sell_order, self.stoploss_price, "unfilled_stoploss"
            )
            self.SL_placed = False
            self.stoploss_price = 0.0
        else:
    ...
    def _stop_loss(self, q, price):
        try:
            self.SL_order = self.client.create_order(
                symbol=self.symbol,
                side=SIDE_SELL,
    ...
    def _market_buy(self, q):
        try:
            order_t = time()
            self.buy_order = self.client.order_market_buy(
                symbol=self.symbol, quantity=q
            )
            ...
            print(
                f" {dt.today()} BUY_MARKET q:{q} self.balance:{self.balance} self.q:{self.q}"
            )
            return True
        except Exception as e:
    ...
    def _market_sell(self, q):
        try:
            order_t = time()
            self.sell_order = self.client.order_market_sell(
                symbol=self.symbol, quantity=q
            )
            ...
            self.q = "0.00000"
            print(
                f" {dt.today()} SELL_MARKET q:{q} self.balance:{self.balance} self.q:{self.q}"
            )
            return True
        except Exception as e:
...
class FuturesTaker:
    def __init__(
        ...
        # Leverage change
        try:
            self.client.change_leverage(symbol=self.symbol, leverage=str(self.leverage))
            self.logger.info(f"Leverage changed to: {self.leverage}x")
        except Exception as e:
            self.logger.error(f"Changing leverage error: {e}")
        # Margin type change
        try:
            m_type = self.client.change_margin_type(
        ...
        self.buy_order, self.sell_order, self.SL_order, self.TP_order = (
            None,
            None,
            None,
            None,
        ...
        self.logger.debug(
            f"prev_data[-5:]: {asarray(self.OHLCV_data)[-5:, :]}, len: {len(self.OHLCV_data)}"
        )
    ...
    def on_message(self, ws, message):
        ...
        if self.SL_order is not None:
            if (
                ...
                if order["status"] != "FILLED":
                    self.logger.warning(
                        f'STOP_LOSS was not filled. orderID:{self.SL_order["orderId"]}'
                    )
        ...
        if self.TP_order is not None:
            if (
                ...
                if order["status"] != "FILLED":
                    self.logger.warning(
                        f'TAKE_PROFIT was not filled. orderID:{self.TP_order["orderId"]}'
                    )
    ...
    def _analyze(self):
        ...
        if self.in_long_position and (self.signal <= -self.long_close_at):
            ...
            if self.signal >= self.long_enter_at:
                ...
                if self._market_buy(q):
                    self.stoploss_price = round_step_size(
                        self.close * (1 - self.stop_loss), self.tick_size
                    )
                    self._stop_loss(q, self.stoploss_price, "SELL")
                    self.takeprofit_price = round_step_size(
                        self.close * (1 + self.take_profit), self.tick_size
                    )
    ...
    def _check_signal(self):
        self.signal = 0.0
        self.logger.debug(
            f"(_analyze to _check_signal: {(time() - self.analyze_t) * 1_000}ms)"
        )
    ...
    def _report_slipp(self, order, req_price, order_type):
        ...
        while _order["status"] != "FILLED":
            _order = self.client.query_order(
                symbol=self.symbol, orderId=order["orderId"]
            )
            retry += 1
            if retry > 10:
                self.logger.debug(
                    f"Order was not filled after {retry} tries at _report_slipp() {_order}"
                )
                break
        if order_type == "buy":
            file = self.buy_slipp_file
    ...
    def _stop_loss(self, q, price, side):
        try:
            self.SL_order = self.client.new_order(
                symbol=self.symbol,
                side=side,
                type="STOP_MARKET",
                # quantity=q,
    ...
    def _take_profit(self, q, price, side):
        try:
            self.TP_order = self.client.new_order(
                symbol=self.symbol,
                side=side,
            ...
            self.logger.info(
                f"TAKE_PROFIT_MARKET q:{q} stopPrice:{price} {dt.today()} "
            )
            return True
        except Exception as e:
    ...
    def _market_buy(self, q):
        try:
            order_t = time()
            self.buy_order = self.client.new_order(
                symbol=self.symbol, side="BUY", type="MARKET", quantity=q
            )
            self.logger.debug(f"(sending buy order: {(time() - order_t) * 1_000}ms)")
            self.logger.debug(
                f"(_analyze to _market_buy: {(time() - self.analyze_t) * 1_000}ms)"
            )
            self.logger.info(
                f"BUY_MARKET q:{q} position_balance:{self.position_balance} {dt.today()}"
            )
            return True
        except Exception as e:
    ...
    def _market_sell(self, q):
        try:
            order_t = time()
            self.sell_order = self.client.new_order(
                symbol=self.symbol, side="SELL", type="MARKET", quantity=q
            )
            self.logger.debug(f"(sending sell order: {(time() - order_t) * 1_000}ms)")
            self.logger.debug(
                f"(_analyze to _market_sell: {(time() - self.analyze_t) * 1_000}ms)"
            )
            self.logger.info(
                f"SELL_MARKET q:{q} position_balance:{self.position_balance} {dt.today()}{dt.today()}"
            )
            return True
        except Exception as e:
    ...
    def _collect_previous_candles(self, itv, prev_size, multi):
        ...
        while len(prev_candles) < prev_size * multi:
            fetched_candles = self.client.klines(
                symbol=self.symbol, interval=itv, limit=limit, startTime=_start_time
            )
        ...
        prev_data[where(prev_data[:, -2] == 0.0), -2] = 0.00000001
        return deque(prev_data, maxlen=len(prev_candles[:-1]))
