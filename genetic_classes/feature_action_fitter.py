...
class MACDFitting(ElementwiseProblem):
    def __init__(self, df, target_segments=None, min_match_factor=0.01, mode='match', *args, **kwargs):
        ...
        macd_variables = {
            "signal_type": Choice(
                options=[
                    "lines_cross_with_zero",
                    "lines_cross",
                    "lines_approaching_cross_with_zero",
                    "lines_approaching_cross",
                    "signal_line_zero_cross",
...
class StochasticOscillatorFitting(ElementwiseProblem):
    def __init__(self, df, target_segments=None, min_match_factor=0.01, mode='match', *args, **kwargs):
        ...
        stoch_variables = {
            ...
            "fastK_period": Integer(bounds=(2, 500)),
            "slowK_period": Integer(bounds=(1, 500)),
            "slowD_period": Integer(bounds=(2, 500)),
...
class ChaikinOscillatorFitting(ElementwiseProblem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        # print(f'X {X}')
        chaikin_oscillator = custom_ChaikinOscillator_cached(
            fast_ma_type=X["fast_ma_type"],
            fast_period=X["fast_period"],
            slow_ma_type=X["slow_ma_type"],
            slow_period=X["slow_period"],
...
class MACDFitting_v1(ElementwiseProblem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        if self.signals_source == "cross":
            ...
            raise ValueError(
                "Unknown signals source, available {cross, zero-cross, histogram-reversal} "
            )
        # signals = where(signals >= 1, 1, where(signals <= -1, -1, 0))
...
class StochasticOscillatorFitting_v1(ElementwiseProblem):
    def __init__(self, df, lower, upper, *args, signals_source="cross", **kwargs):
        ...
        if self.signals_source == "mid-cross":
            ...
            raise ValueError(
                "Unknown signals source, available {cross, threshold, mid-cross} "
            )
        super().__init__(*args, vars=stoch_variables, n_obj=1, **kwargs)
        ...
        if self.signals_source == "cross":
            ...
            signals = array(
                StochasticOscillator_mid_cross_signal(
                    slowK, slowD, mid_level=X["mid_level"]
                )
            )
        else:
            raise ValueError(
                "Unknown signals source, available {cross, threshold, mid_cross} "
            )
        # signals = where(signals >= 1, 1, where(signals <= -1, -1, 0))
        # out["F"] = euclidean(nan_to_num(signals[mask]), self.actions[mask])
        out["F"] = mean_squared_error(
...
class ADXFitting_v1(ElementwiseProblem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        # print(f'slowD quantiles: {percentile(slowD, [0, 50, 100])}', end='\n')
        if self.signals_source == "cross":
            signals = array(ADX_signal(adx_col=adx, minus_di=minus_DI, plus_di=plus_DI))
        elif self.signals_source == "trend":
            signals = array(
                ADX_trend_signal(adx_col=adx, minus_di=minus_DI, plus_di=plus_DI)
            )
        else:
...
class ChaikinOscillatorFitting_v1(ElementwiseProblem):
